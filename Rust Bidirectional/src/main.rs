...
fn build_vocab(rows: &[Row], min_freq: usize) -> Vocab {
    ...
    for r in rows {
        for t in simple_tokenize(&r.text, &re) {
    ...
    for (tok, c) in pairs {
        if c >= min_freq { vocab.add_token(&tok); }
    }
    vocab
}
...
impl BiLstmClf {
    fn new(
        ...
        let lstm_cfg = nn::RNNConfig { bidirectional: true, num_layers, ..Default::default() };
        let lstm = nn::lstm(vs / "lstm", emb_dim, hidden, lstm_cfg);
        let fc = nn::linear(vs / "fc", hidden * 2, num_classes, Default::default());
    ...
    fn forward(&self, xs: &Tensor, lens: &Tensor, train: bool) -> Tensor {
        ...
        let out = out.transpose(0, 1); // (batch, seq, hidden*2)
...
fn main() -> Result<()> {
    ...
    eprintln!("Device: {:?} | Mode: bidirectional | layers={} hidden={}", device, cfg.num_layers, cfg.hidden);
    ...
    let root = &vs.root();
    let model = BiLstmClf::new(root, vocab.len(), cfg.emb_dim, cfg.hidden, cfg.num_layers, num_classes, cfg.dropout);
    ...
    nn::VarStore::save(&vs, "checkpoints/bilstm_textclf.ot")?;
    Ok(())
}
...
fn split_rows(rows: &[Row], train_ratio: f64, val_ratio: f64, seed: u64) -> (Vec<Row>, Vec<Row>, Vec<Row>) {
    ...
    let train_idx = &idx[..n_train.min(n)];
    let val_idx = &idx[n_train.min(n)..(n_train + n_val).min(n)];
    let test_idx = &idx[(n_train + n_val).min(n)..];
    let take = |ids: &[usize]| -> Vec<Row> { ids.iter().map(|&i| rows[i].clone()).collect() };
    (take(train_idx), take(val_idx), take(test_idx))
}
